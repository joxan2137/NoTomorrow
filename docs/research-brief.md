...
## 8. Stack decisions after the second research pass (2026-09-04)

- **Backend on Fly.io, region `ams`** (Warsaw is not a Fly region; ams is the cheapest EU region, Managed Postgres exists there). Node 22 + Hono + `postgres` (porsager) or Drizzle, `@parse/node-apn` 8.x for APNs, `jose` for Apple/Google JWKS verification, `pg-boss` for the 21:00 "did you skip?" job (`TZ` per user, `min_machines_running = 1`, no scale-to-zero). Unmanaged single-node Fly Postgres (~$2/mo) + daily volume snapshots + nightly `pg_dump`; move to Managed Postgres Basic ($38/mo) with paying users. Total ≈ $4–6/mo. Realtime: APNs alert push is the source of truth; SSE only while the Bro screen is open (heartbeat every 25 s, `idle_timeout = 600`); always pull on foreground. No WebSocket. Fly free trial exists (2 VM-hours or 7 days) but is not a free tier.
- **AI default: `gemini-3.1-flash-lite`** (stable, $0.25/M in, $1.50/M out incl. thinking, 1,120 tokens per image at default resolution → ≈ $0.0009 per photo), paid tier only (free tier trains on data), through the backend with the Interactions API (`POST /v1beta/interactions`, `input` array with text + image, `response_format` JSON schema). Cap thinking. Request Zero Data Retention. Published benchmarks show Gemini Flash recognizes food well but quantifies portions worse than Claude; build a 100–200 photo eval set before shipping.
- **Claude as premium = bring your own Anthropic API key.** Anthropic explicitly prohibits third-party apps from offering Claude.ai login or routing consumer-subscription credentials; there is no entitlement API; the Agent SDK subscription credit is paused. So: Settings → AI estimates → "Standard (Gemini)" / "Claude with your API key". Key stored in Keychain (`kSecAttrAccessibleWhenUnlockedThisDeviceOnly`, not synchronizable), calls go device → `api.anthropic.com` directly, never through our backend, model Sonnet 5 ($2/$10; Haiku 4.5 cannot cache our short prompt, Sonnet needs ≥1,024 tokens to cache either). Label it "Anthropic API key", never "sign in with Claude"; no "buy credits" links (App Store 3.1.3); default path must work with zero setup (2.1(a)); consent sheet naming the provider before the first photo leaves the device (5.1.2(i)). Google has the same answer: no user-pays OAuth.
- **Auth**: Sign in with Apple (required once Google Sign-In is offered, rule 4.8) + Google Sign-In iOS SDK 10.0.0 (SPM `GoogleSignIn`, `GIDClientID` + reversed-client-id URL scheme, send the ID token, verify against `https://www.googleapis.com/oauth2/v3/certs`, key on `sub`) + username/password (email optional but prompted as recovery; Argon2id m=19456,t=2,p=1; NIST 800-63B-4: 15-char minimum single factor, no composition rules, breach blocklist, account-scoped rate limit, generic errors, constant-time). Apple: verify identity token against `https://appleid.apple.com/auth/keys` (RS256), `aud` = bundle id, exchange the code at `/auth/token` with an ES256 client secret, store the refresh token, validate at most daily, and call `/auth/revoke` on account deletion (TN3194); handle server-to-server `consent-revoked`/`account-deleted`. Sessions: 15-min access JWT + opaque rotating refresh token (32 random bytes) in Keychain (`AfterFirstUnlockThisDeviceOnly`). One `users` row, many `identities (provider, subject)` rows; never auto-merge by email (Apple relay addresses).
