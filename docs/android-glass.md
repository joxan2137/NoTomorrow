...
7. **No chromatic aberration, no dispersion, no depth term on resting chrome.** iOS shows none in the interior (§1.1), and each costs 3–7 taps per pixel instead of 1. The one exception is the tab bar's selection pill *while it moves or settles*: iOS splits it into a rainbow there, so `GlassStyle.pillLens` samples seven spectral bands (rim band + travel drift + a small radial term; the colour outlasts the bend into the settle, then cuts off before the spring's tail). The taps sit behind a uniform branch, so every other glass node and the pill at rest still pay one.
