...
export function aiRoutes(deps: AppDeps): Hono<AppEnv> {
  ...
  r.post('/ai/estimate', async (c) => {
    ...
    const fields = fieldsSchema.safeParse({ meal: form.meal, locale: form.locale ?? 'en' });
    ...
    try {
      const estimate = await estimateFood(
        deps.env.gemini,
        { imageBase64: Buffer.from(bytes).toString('base64'), meal: fields.data.meal, locale: normalizeLocale(fields.data.locale) },
        deps.fetchImpl,
      );
      return c.json({ foods: estimate.foods, overallConfidence: estimate.overallConfidence });
    } catch (err) {
