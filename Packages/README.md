...
| `MetalFxKit` | `packages/metal-fx/ports/ios/MetalFxKit` | Exercise picker: the search bar's chromatic ring and its metal "add" circle, the "Add n" pill, and the silver ring of selected result cards |

Local changes:

- The test targets were dropped from the manifests (their golden files live elsewhere in the
  upstream repo). The READMEs are upstream's and still mention those tests and scripts.
- `MetalFxKit`'s `MetalSheetMapping` caps a shape's window at 72 % × 85 % of the material sheet
  (`maxWindowX` / `maxWindowY`). Upstream lets a shape larger than the canonical 140 × 40 pt reach
  the sheet's fading edges, so a full-width field or card lost its ring at the ends. Shapes of the
  canonical size or smaller are unaffected.
...
- `BorderBeam.kt` and `LiquidMetal.kt` run upstream's React Native SkSL shaders as AGSL (API 33+)
  with the same numbers; below API 33 the beam is left out and the metal band is a still gradient.
  `MetalMaterial.kt` (presets, the capped sheet mapping, a CPU port of the material) and
  `MetalGlow.kt` (the wandering halo and its sprites) complete the MetalFx look on every API level.
